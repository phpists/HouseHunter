  ...
  return (
    <StyledCard hide={hide} ref={cardRef} className={`${hide && "hide-card"}`}>
      <Slider
        ...
        price={data?.[`price_${currency?.toLowerCase()}`] ?? 0}
        showLike={showLike}
        status={status}
      ...
      <div className="flex flex-col justify-between card-content">
        <div className="card-text">
          ...
          <Tags
            doors={data?.rooms && data?.rooms !== "0" ? `${data?.rooms}к` : ""}
            stairs={
              data?.address_storey !== "0" || data?.storey_count !== "0"
                ? `${
                    data?.address_storey === "0" ? "-" : data?.address_storey
                  } із ${data?.storey_count === "0" ? "-" : data?.storey_count}`
                : ""
            }
            areaTotal={
              data?.area_total && data?.area_total !== "0"
                ? data?.area_total
                : ""
            }
            areaKitchen={
              data?.area_kitchen && data?.rooms !== "0"
                ? data?.area_kitchen
                : ""
            }
          />
